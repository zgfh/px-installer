...
func generate(etcd, cluster, dataIface, mgmtIface, drives, zeroStorage, force, etcdPasswd,
	etcdCa, etcdCert, etcdKey, acltoken, token, env string) string {

	cwd, _ := os.Getwd()
	p := filepath.Join(cwd, "k8s-pxd-spec-response.gtpl")
...
func main() {
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, generate(etcd, cluster, dataIface, mgmtIface, drives, zeroStorage, force,
			etcdPasswd, etcdCa, etcdCert, etcdKey, acltoken, token, env))
	})
